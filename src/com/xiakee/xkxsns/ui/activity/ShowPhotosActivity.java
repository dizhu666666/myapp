...
public class ShowPhotosActivity extends Activity {
	...
	private class PreviewListener implements OnClickListener {
		...
		public void onClick(View v) {
			if (Bimp.tempSelectBitmap.size() > 0) {
				...
				intent.setClass(ShowPhotosActivity.this, PreviewActivity.class);
	...
	private class BackListener implements OnClickListener {// 返回按钮监听
		...
		@Override
		public void onClick(View v) {
			intent.setClass(ShowPhotosActivity.this, FoldersActivity.class);
			startActivity(intent);
		}
	...
	private void initListener() {
		...
		gridImageAdapter.setOnItemClickListener(new AlbumGridViewAdapter.OnItemClickListener() {
			@Override
			public void onItemClick( int position, boolean isChecked, ImageButton selectButton) {
				if (Bimp.tempSelectBitmap.size() >= PublicWay.num && isChecked) {
					selectButton.setSelected(false);
//					toggleButton.setChecked(false);
					Toast.makeText(ShowPhotosActivity.this, Res.getString("only_choose_num"), 200).show();
					return;
				}
		...
		okButton.setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				...
				// IntentFilter("data.broadcast.action");
				// registerReceiver(broadcastReceiver, filter);
